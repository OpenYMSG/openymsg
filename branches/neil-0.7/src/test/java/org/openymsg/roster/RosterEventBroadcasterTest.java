...
/**
 * Tests to check the functionality of the mechanism that should broadcast any given {@link RosterEvent} to all
 * registered Listeners. This testcase tests the broadcast-method itself, rather than checking if add() and remove()
 * functions trigger a broadcast. For that, refer to other testcases like {@link RosterSyncedAddMethodTriggersUpdate}.
 * 
...
public class RosterEventBroadcasterTest {
    ...
    public void testReceiveOnOneRegisteredListener() {
        ...
        roster.broadcastEvent(new RosterEvent(roster, new YahooUser("dummy"), RosterEventType.add));
