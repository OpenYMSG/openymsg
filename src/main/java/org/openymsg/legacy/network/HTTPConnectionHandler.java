...
public class HTTPConnectionHandler extends ConnectionHandler {
	 ...
	 * @param proxyHost Host to use.
	 * @param proxyPort Port to use.
	 * @param blacklist Option set of hosts explicitly forbidden to use.
	 */
	 ...
	 * by zero or more packets. The first byte of the four is a count of packets encoded in the following body. Each
	 * incoming packet is transfered to a queue, where receivePacket() takes them off - thus preserving the effect that
	 * input and output packets are being received independently, as with other connection handlers. As readPacket() can
	 * throw an exception, these are caught and transfered onto the queue too, then rethrown by receivePacket() .
	 */
	@Override
	 ...
	 * true if this packet should be surpressed.
	 * @throws IOException
