...
public abstract class Util implements PropertyConstants {
	...
	/*
	 * public static String getPropString(String key,String def) { return System.getProperty(key,def); } public static
	 * int getPropInt(String key,int def) { String v = System.getProperty(key); if(v==null) return def; try { return
	 * Integer.parseInt(v); } catch(NumberFormatException e) { return def; } } public static int[] getPropIntA(String
	 * key,int[] def) { String v = System.getProperty(key); if(v==null) return def; StringTokenizer st = new
	 * StringTokenizer(v,","); int[] arr = new int[st.countTokens()]; int cnt=0; while(st.hasMoreTokens()) { try {
	 * arr[cnt++]=Integer.parseInt(st.nextToken()); } catch(NumberFormatException e) {} } return arr; }
	 */
	/**
	...
	private final static String[] ENTITIES_STR =
			{"apos", "quot", "amp", "lt", "gt", "nbsp", "curren", "cent", "pound", "yen", "copy"};
	 ...
	 * @param table The hashtable to look in for 'key'.
	 * @param key The key for which the value should be returned.
	 * @param defaultValue Value to return if 'key' doesn't exist in 'table'.
