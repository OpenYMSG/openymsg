 ...
 * invite received/accepted to logoff). Packets which arrive before an invite are buffered. When an invite arrives the
 * packets are fetched and the buffer null'd (which is then used as a flag to determine whether an invite has arrived or
 * not). By using this method, the API user will *ALWAYS* get an invite before any other packets. The closed flag marks
 * a closed conference. Packets arriving after this time should be ignored. The users list should not contain any of our
 * user's own identities. This is why they are screened out by the addUser/addUsers methods.
...
public class YahooConference // Cannot be serialised
{
	...
	public String toString() {
		...
		try {
			sb.append(room).append(" users=").append(users.size()).append(" id=").append(identity.getId())
	 ...
	 * @param usernames Names of all users to add
	 */
