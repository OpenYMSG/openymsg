 ...
 * version eleven was already on the scene. I think it is more correct to label this challenge/response code as version
 * ten. (The current v11 probably has some hidden gems which won't be activated until twelve is with us, and ten is
 * finally end-of-line'd!) If you study other YMSG libraries you should be aware of the labeling differences. Yahoo has
 * repeatedly tweaked their login protocol, by designing the challenge/response code in such a way that aspects of the
 * algorithm will remain unused until the challenge key is tweaked to cause them to swing into effect. The major change
 * came around September 2003, when Yahoo dropped support for version nine clients, allowing them to switch on a few
 * nasties they had added into the version ten c/r code. Then in January 2004 they switched on a new way of encoding
 * their messages using a fake mathematical expression. Again in June 2004 they tweaked the c/r to take advantage of a
 * flaw in the way many third party libraries had implemented the algorithm (most of whom base their code on work done
 * by the Gaim and Trillian projects). Yahoo claim that they do these things to protect their users from spam and the
 * like - when will they learn that third party developers are (almost always) not the enemy...? They don't like
 * spammers any more than Yahoo do (indeed many of them have added advanced filtering and security technologies into
 * their clients above-and-beyond that which Yahoo supports). If only they would stop wasting so much time fighting
 * third party developers, and co-operate with them, so together we can devise ways of shutting out spammers for good!
...
public class ChallengeResponseV10 extends ChallengeResponseUtility implements ChallengeResponseV10Tables {
	...
	private final static String BINARY_DATA = "/legacy/challenge.bin";
	 ...
	 * to Yahoo.
	 * @param username Username of the session that is trying to authenticate
	 * @param password Password that validates <tt>username</tt>
	 * @param challenge The challenge as received from the Yahoo network.
