 ...
 * of users. NOTE: this is the second implementation of this class. The original used Yahoo's old method of accessing
 * category/room data. They have now dropped that scheme in favour of an XML based approach. Categories are modelled by
 * the YahooChatCategory class, rooms by the YahooChatRoom class, and lobbies by (shock horror!) the YahooChatLobby
 * class. The data is delivered from Yahoo via a call the below URL, and in the following format :
 * http://insider.msg.yahoo.com/ycontent/?chatcat=0 Resulting in (indented to improve readability) ...
 * 
 * <pre>
 ...
 * http://insider.msg.yahoo.com/ycontent/?chatroom_&lt;id&gt; Resulting in (indented for readability)...
 * 
 * <pre>
 ...
 * changes radically. NOTE: this class used to rely on a home-brew HTTP class called openymsg.network.HTTPConnection .
 * This was because Yahoo had a nasty habit of sending back HTTP responses with no blank line between the header and the
 * opening XML line (invalid, in other words!) As Sept 2006 (v0.7) the problem appears to have been fixed, so this code
 * has been converted to use the regular java.net.* HTTP classes.
...
public class YahooChatCategory {
	 ...
	 * @param category the new subcategory of this category.
	 */
	...
	public synchronized void loadRooms() throws Exception {
		...
		for (Element room : rooms) {
			...
			final YahooChatRoom yahooChatRoom =
					new YahooChatRoom(roomId, roomName, roomTopic, roomType.equals(PUBLIC_TYPE));
