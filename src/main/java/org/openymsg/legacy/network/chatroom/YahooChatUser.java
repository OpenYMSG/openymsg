...
/**
 * This class wraps a regular YahooUser to provide the extra information provided for each chat user. Note: if a
 * YahooUser object for this user does not exist, one is automatically created and added to the static users hash in
 * YahooUser.
...
public class YahooChatUser extends YahooUser // Cannot be serialised
{
	...
	public String toString() {
		StringBuffer sb = new StringBuffer(super.toString()).append("] age=").append(age).append(" attributes=")
				.append(Integer.toHexString(attributes)).append(" alias=").append(alias).append(" location=")
				.append(location);
		return sb.toString();
	}
