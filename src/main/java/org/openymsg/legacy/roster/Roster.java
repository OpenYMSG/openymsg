...
import org.openymsg.legacy.addressBook.YahooAddressBookEntry;
import org.openymsg.legacy.network.ContactListType;
import org.openymsg.legacy.network.FireEvent;
import org.openymsg.legacy.network.FriendManager;
...
public class Roster implements Set<YahooUser>, SessionListener {
	...
	/**
	 * A collection of all YahooUsers on the roster. The YahooUsers are mapped by the value returned by
	 * {@link YahooUser.getId()}
	 */
	private final Map<String, YahooUser> usersById = new ConcurrentHashMap<String, YahooUser>();
	/**
	 * A collection of all YahooUsers from address book. The YahooUsers are mapped by the value returned by
	 * {@link YahooUser.getId()}
	 */
	private final Map<String, YahooAddressBookEntry> addressBookUsersById =
			new Hashtable<String, YahooAddressBookEntry>();
	/**
	 ...
	 * Yahoo network.
	 * @param manager The object used to relay changes made to this Roster to the Yahoo network.
	 */
	 ...
	 * @param listener The new listener that gets notified of any roster changes.
	 */
	 ...
	 * @param event The event that is going to be broadcasted to all registered event listeners.
	 */
	 ...
	 * @param user The YahooUser to be added to this roster.
	 ...
	 * @throws IllegalArgumentException if some aspect of the specified user prevents it from being added to this set.
	 */
	@Override
	 ...
	 * @param userObject user object to be removed from this roster, if present.
	 * @return <tt>true</tt> if the roster contained the specified user.
	 * @throws ClassCastException if the type of the specified user object is not a YahooUser instance.
	 ...
	 * @param userId ID of the user to be removed from this roster, if present.
	 * @return <tt>true</tt> if the roster contained the specified user.
	 ...
	 * @param userId ID of the user to be removed from this roster, if present.
	 * @throws NullPointerException if the specified user object is null.
	 * @throws IllegalArgumentException If the userId String is null or empty, or if user.getId() does not match the
	 *         userId provided.
	 * @throws IllegalStateException If the userId does not match a user that's currently on this roster.
	 */
	 ...
	 * @param user object whose presence on this roster is to be tested.
	 * @return <tt>true</tt> if this set contains the specified element.
	 * @throws ClassCastException if the type of the specified element is incompatible with this set (not a YahooUser).
	 ...
	 * @param userId ID of a user whose presence on this roster is to be tested.
	 * @return <tt>true</tt> if this set contains the specified element.
	 * @throws NullPointerException if the specified userId is null.
	 * @throws IllegalArgumentException if the specified userId is an empty String.
	 */
	 ...
	 * @throws UnsupportedOperationException removeAll is not supported by this set.
	 */
	@Override
		...
		switch (event.getType()) {
			...
			case Y7_AUTHORIZATION:
				if (fEvent instanceof SessionFriendAcceptedEvent) {
					...
					log.info("Ignoring SessionFriendEvent of type " + event.getType()
							+ " that contains an event that we" + " do not know how to process: " + fEvent);
				}
				break;
			default:
				log.info("Ignoring SessionFriendEvent that came" + " with an unsupported ServiceType: "
						+ event.getType());
				break;
		}
	}
