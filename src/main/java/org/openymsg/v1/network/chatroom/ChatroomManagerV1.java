 ...
 */
public class ChatroomManagerV1 implements ChatroomManager<YahooChatCategoryV1, YahooChatLobbyV1> {
	/**
	 ...
	 * 
	 */
	private final static Map<String, YahooChatCategoryV1> roots = new HashMap<String, YahooChatCategoryV1>();

	/**
	 * map of timestamp values indicating when the specified localePrefix was
	 * last updated;
	 * 
	 */
	...
	public ChatroomManagerV1(String[] sessionCookies, String localePrefix) {
		if (sessionCookies != null) {
			cLine = /* FIXME "Cookie: "+ */
			sessionCookies[NetworkConstants.COOKIE_Y] + "; "
					+ sessionCookies[NetworkConstants.COOKIE_T];
		} else {
			cLine = null;
		}

		this.localePrefix = (localePrefix == null) ? "" : (localePrefix
				.endsWith(".") ? localePrefix : localePrefix + ".");

	}

	/**
	 * Recursively loads all YahooCategories that are sub-categories of the
	 * provided parent category.
	 * 
	 * @param parentElement
	...
	private void recursiveLoad(Element parentElement,
			YahooChatCategoryV1 parentCategory, String cookieLine) {
		...
		for (Element e : childCategories) {
			...
			final YahooChatCategoryV1 yahooCategory = new YahooChatCategoryV1(id,
	 ...
	 */
	public synchronized YahooChatCategoryV1 loadCategories()
		...
		final YahooChatCategoryV1 root = new YahooChatCategoryV1(Long
				.parseLong(content.getAttributeValue("time")), "<root>", cLine,
				localePrefix);
	...
	public YahooChatLobbyV1 getLobby(String networkName) {
		final YahooChatCategoryV1 root = roots.get(localePrefix);
		if (root == null) {
			throw new IllegalStateException("Root category for this locale ("
					+ localePrefix + ") has not been loaded yet.");
		}
