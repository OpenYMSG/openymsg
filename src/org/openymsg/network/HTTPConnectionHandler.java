...
public class HTTPConnectionHandler extends ConnectionHandler {
	...
	private static final String HTTP_HEADER_POST = "POST http://"
			+ Util.httpHost() + "/notify HTTP/1.0" + NetworkConstants.END;

	private static final String HTTP_HEADER_AGENT = "User-Agent: "
			+ NetworkConstants.USER_AGENT + NetworkConstants.END;
	...
	/**
	 * Creates a new instance based on the HTTP proxy settings as configured in
	 * property settings.
	 */
	...
	/**
	 * Creates a new instance, using tshe host and port as configured. This
	 * constructor may contain a collection of hosts of hosts to not proxy.
	 * 
	 * @param proxyHost
	 *            Host to use.
	 * @param proxyPort
	...
	public HTTPConnectionHandler(String proxyHost, int proxyPort,
		...
		if (blacklist != null) {
			...
			System.getProperties().put(NetworkConstants.PROXY_NON,
					sb.toString());
		}
	}
	...
	/**
	 * High level method for setting proxy authorization, for users behind
	 * firewalls which require credentials to access a HTTP proxy.
	 */
	public static void setProxyAuthorizationProperty(String method,
	...
	/**
	 * The only time Yahoo can actually send us any packets is when we send it
	 * some. Yahoo encodes its packets in a POST body - the format is the same
	 * binary representation used for direct connections (with one or two extra
	 * codes).
	 * 
	 * After posting a packet, the connection will receive a HTTP response who's
	 * payload consists of four bytes followed by zero or more packets. The
	 * first byte of the four is a count of packets encoded in the following
	 * body.
	 * 
	 * Each incoming packet is transfered to a queue, where receivePacket()
	 * takes them off - thus preserving the effect that input and output packets
	 * are being received independently, as with other connection handlers. As
	 * readPacket() can throw an exception, these are caught and transfered onto
	 * the queue too, then rethrown by receivePacket() .
	 */
	@Override
	synchronized void sendPacket(PacketBodyBuffer body, ServiceType service,
		...
		if (s == null || s.indexOf(" 200 ") < 0) // Not "HTTP/1.0 200 OK"
		{
			throw new IOException(
					"HTTP request returned didn't return OK (200): " + s);
		}
			...
			if (!filterInput(pkt)) {
				if (!packets.add(pkt)) {
					throw new IllegalArgumentException(
							"Unable to add data to the packetQueue!");
				}
			}
		}
	...
	/**
	 * This method blocks until there is a packet to deliver, during which time
	 * it releases its object lock.
	 */
	@Override
	...
	/**
	 * Sometimes packets need to be altered, either going in or out. Typically
	 * this is about reading or adding extra HTTP specific content which Yahoo
	 * uses. These methods are called to perform the necessary manipulations.
	 * 
	...
	private boolean filterInput(YMSG9Packet pkt) {
		...
		case LIST:
			...
			cookie = cookieArr[NetworkConstants.COOKIE_Y] + "; "
					+ cookieArr[NetworkConstants.COOKIE_T];
			break;
		case LOGON:
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("HTTP connection: ").append(
				proxyHost).append(":").append(proxyPort);
		return sb.toString();
	}

	/**
	 * This thread fires off a IDLE packet every thirty seconds. This is because
	 * the only way the server can deliver us any incoming packets is on the
	 * input stream of a HTTP connection we have made ourselves.
	 */
